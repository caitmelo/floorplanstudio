...
Recovered source for an iPhone rental inspection app. Includes ingoing/outgoing inspections, condition notes, evidence photographs, guided multi-photo panorama capture, a panorama viewer, local signatures and PDF export. These are implemented code paths, not a claim of successful device testing.

This is TypeScript/React Native with Expo SDK 57, not Python. The repository name is `floorplanstudio`; the app is RoomRecord. Version 0.3.1 adds reproducible source checks and an audit; it does not complete the original native LiDAR app.
...
The screen should show **0.3.1** beside “On this phone.” A Snack error referring to `/src/model.ts` does not establish that this repository build was opened. Do not reuse earlier anonymous Snack QR codes as verification of this commit.
