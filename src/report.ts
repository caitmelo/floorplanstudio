...
export async function reportHTML(property: Property, inspection: Inspection) {
  ...
  for (const room of inspection.rooms) {
    ...
    for (const photo of room.evidence) {
      ...
      photos.push(
        `<figure><img src="${data}"/><figcaption>${e(photo.kind === "panorama" ? "Panorama · flattened view" : "Photo")} · ${e(new Date(photo.capturedAt).toLocaleString())}<br/>${e(photo.caption)}${photo.coverage !== undefined ? `<br/>Image coverage: ${(photo.coverage * 100).toFixed(1)}%. Black regions are uncaptured.` : ""}</figcaption></figure>`,
      );
    }
    rooms.push(
